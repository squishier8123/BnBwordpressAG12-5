...
jQuery(document).on('submit', '.afeb-form', function(e) {
    e.preventDefault();
    let form = jQuery(this);
    let fieldOrder = JSON.parse(form.find('[name="field_order"]').val() || '[]');

    // Reorder submitted data according to fieldOrder
    let formData = new FormData(this);
    let orderedData = {};

    fieldOrder.forEach(fieldId => {
        if (formData.has('form_fields['+fieldId+']')) {
            orderedData[fieldId] = formData.get('form_fields['+fieldId+']');
        }
    });

    // Submit the ordered data
    jQuery.ajax({
        url: afeb_ajax.ajax_url,
        type: 'POST',
        data: {
            action: 'afeb_form_submit',
            field_order: fieldOrder,
            form_fields: orderedData,
...
jQuery(document).ready(function($) {
    $(document).on('submit', '.afeb-form', function(e) {
        ...
        $form.find('[name^="form_fields"]').each(function() {
            var fieldId = this.name.match(/\[(.*?)\]/)[1];
            if (fieldId) fieldOrder.push(fieldId);
        });
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    if (response.data.redirect) {
                        ...
                        $form.find('.afeb-form-message').html('<div class="afeb-form-success">' + response.data.message + '</div>');
                        $form[0].reset();
                    }
                } else {
                    // Handle errors
                    if (response.data.errors) {
                        $.each(response.data.errors, function(fieldId, error) {
                            $form.find('.afeb-field-group-' + fieldId).append('<div class="afeb-field-error">' + error + '</div>');
                        });
                    } else {
                        $form.find('.afeb-form-message').html('<div class="afeb-form-error">' + response.data + '</div>');
                    }
                }
            },
            error: function(xhr) {
                $form.find('.afeb-form-message').html('<div class="afeb-form-error">An error occurred. Please try again.</div>');
            }
        });
    });
});
