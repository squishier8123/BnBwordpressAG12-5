...
/**
 * "Vertex Addons for Elementor" WPImport Class
 * 
 * @class WPImport
 * @version 1.4.0
 */
class WPImport extends WP_Importer
{
	...
	{
		if (!is_file($file)) {
			$this->output['errors'] = [esc_html__('The import file does not exist, Please make sure the file address is correct.', 'addons-for-elementor-builder')];
		...
		if (!empty($import_data['authors'])) {
			...
			foreach ($import_data['posts'] as $post) {
				...
				if (empty($login)) {
					...
					$this->output['errors'][] = sprintf(esc_html__('Failed to import author %s. Their posts will be attributed to the current user.', 'addons-for-elementor-builder'), $post['post_author']);
					continue;
				}
		...
		foreach ((array) $this->args['imported_authors'] as $i => $old_login) {
			...
			if (!empty($this->args['user_map'][$i])) {
				...
				if (!is_wp_error($user_id)) {
					...
					$error = sprintf(esc_html__('Failed to create new user for %s. Their posts will be attributed to the current user.', 'addons-for-elementor-builder'), $this->authors[$old_login]['author_display_name']);
	...
	private function process_categories()
	{
		...
		foreach ($this->categories as $cat) {
			...
			if (!is_wp_error($id) && $id > 0) {
				...
				$error = sprintf(esc_html__('Failed to import category %s', 'addons-for-elementor-builder'), $cat['category_nicename']);
			...
			add_term_meta($id, '_afeb_demo_import_item', true);
		}
	...
	private function process_tags()
	{
		...
		if (empty($this->tags)) return $result;
		...
		foreach ($this->tags as $tag) {
			...
			if (!is_wp_error($id)) {
				...
				$error = sprintf(esc_html__('Failed to import post tag %s', 'addons-for-elementor-builder'), $tag['tag_name']);
			...
			add_term_meta($id['term_id'], '_afeb_demo_import_item', true);
		}
	...
	private function process_terms()
	{
		...
		foreach ($this->terms as $term) {
			...
			if (!is_wp_error($id)) {
				...
				$error = sprintf(esc_html__('Failed to import %1$s %2$s', 'addons-for-elementor-builder'), $term['term_taxonomy'], $term['term_name']);
	...
	private function process_posts()
	{
		...
		foreach ($this->posts as $post) {
			...
			if (!post_type_exists($post['post_type'])) {
				...
				$this->output['errors'][] = sprintf(esc_html__('Failed to import %1$s: Invalid post type %2$s', 'addons-for-elementor-builder'), $post['post_title'], $post['post_type']);
			...
			if (isset($this->author_mapping[$author])) $author = $this->author_mapping[$author];
			...
			if (is_wp_error($post_id)) {
				...
				$error = sprintf(__('Failed to import %1$s %2$s', 'addons-for-elementor-builder'), $post_type_object->labels->singular_name, $post['post_title']);
				...
				foreach ($post['terms'] as $term) {
					...
					if (!$term_id) {
						...
						if (!is_wp_error($t)) {
							...
							$error = sprintf(esc_html__('Failed to import %1$s %2$s', 'addons-for-elementor-builder'), $taxonomy, $term['name']);
			...
			add_post_meta($post_id, '_afeb_demo_import_item', true);
		}
	...
	private function process_menu_item($item)
	{
		...
		if (!$menu_slug) {
			$this->output['errors'][] = esc_html__('Menu item skipped due to missing menu slug', 'addons-for-elementor-builder');
			return;
		}
		...
		if (!$menu_id) {
			...
			$this->output['errors'][] = sprintf(esc_html__('Menu item skipped due to invalid menu slug: %s', 'addons-for-elementor-builder'), $menu_slug);
	...
	private function process_attachment($post, $url)
	{
		...
		if (! $this->fetch_attachments)
			return new WP_Error('attachment_processing_error', esc_html__('Fetching attachments is not enabled', 'addons-for-elementor-builder'));
		...
		if ($info) {
			...
			return new WP_Error('attachment_processing_error', esc_html__('Invalid file type', 'addons-for-elementor-builder'));
		}
	...
	private function fetch_remote_file($url, $post)
	{
		...
		if (!$tmp_file_name)
			return new WP_Error('import_no_file', esc_html__('Could not create temporary file.', 'addons-for-elementor-builder'));
		...
		if (is_wp_error($remote_response)) {
			...
			return new WP_Error('import_file_error', sprintf( /* translators: 1: WordPress error message, 2: WordPress error code. */esc_html__('Request failed due to an error: %1$s (%2$s)', 'addons-for-elementor-builder'), esc_html($remote_response->get_error_message()), esc_html($remote_response->get_error_code())));
		}
		...
		if (200 !== $remote_response_code) {
			@unlink($tmp_file_name);
			return new WP_Error('import_file_error', sprintf( /* translators: 1: HTTP error message, 2: HTTP error code. */esc_html__('Remote server returned the following unexpected result: %1$s (%2$s)', 'addons-for-elementor-builder'), get_status_header_desc($remote_response_code), esc_html($remote_response_code)));
		}
		...
		if (!$headers) {
			@unlink($tmp_file_name);
			return new WP_Error('import_file_error', esc_html__('Remote server did not respond', 'addons-for-elementor-builder'));
		}
		...
		if (0 === $filesize) {
			@unlink($tmp_file_name);
			return new WP_Error('import_file_error', esc_html__('Zero size file downloaded', 'addons-for-elementor-builder'));
		}

		if (!isset($headers['content-encoding']) && isset($headers['content-length']) && $filesize !== (int) $headers['content-length']) {
			@unlink($tmp_file_name);
			return new WP_Error('import_file_error', esc_html__('Downloaded file has incorrect size', 'addons-for-elementor-builder'));
		}
		...
		if (!empty($max_size) && $filesize > $max_size) {
			...
			return new WP_Error('import_file_error', sprintf(esc_html__('Remote file is too large, limit is %s', 'addons-for-elementor-builder'), size_format($max_size)));
		}
		...
		if ((!$type || !$ext) && !current_user_can('unfiltered_upload'))
			return new WP_Error('import_file_error', esc_html__('Sorry, this file type is not permitted for security reasons.', 'addons-for-elementor-builder'));
		...
		if (!$move_new_file) {
			@unlink($tmp_file_name);
			return new WP_Error('import_file_error', esc_html__('The uploaded file could not be moved', 'addons-for-elementor-builder'));
		}
