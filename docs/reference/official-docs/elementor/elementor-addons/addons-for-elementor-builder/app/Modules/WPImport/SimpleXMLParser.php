...
/**
 * "Vertex Addons for Elementor" SimpleXMLParser Class
 * 
...
class SimpleXMLParser
{
	...
	public function parse($file)
	{
		...
		if (version_compare(PHP_VERSION, '8.0.0', '<') && ! is_null($old_value))
			libxml_disable_entity_loader($old_value); // phpcs:ignore Generic.PHP.DeprecatedFunctions.Deprecated

		if (!$success || isset($dom->doctype))
			return new WP_Error('SimpleXML_parse_error', esc_html__('There was an error when reading this WXR file', 'addons-for-elementor-builder'), libxml_get_errors());
		...
		if (!$wxr_version)
			return new WP_Error('WXR_parse_error', esc_html__('This does not appear to be a WXR file, missing/invalid WXR version number', 'addons-for-elementor-builder'));
		...
		if ($base_blog_url) $base_blog_url = (string) trim($base_blog_url[0]);
		else $base_blog_url = $base_url;
